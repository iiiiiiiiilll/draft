...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, params):
    ...
    n_samples, n_features = X.shape  # number of sample, number of features
    # Unpack parameters
    ...
    # n_programs 由 population_size 以及并行运算决定， see line 480
    # 当前job分到的program的数量
    # 父代中每有一个个体，就对population作一次容量为tournament_size的随机采样
    # 从中选出最优的个体根据参数进行遗传变异，或者保留
    ...
    # 返回当前种群中所有个体，格式为list, list中的元素是由_program类建立的对象
    # 这一步只遗传变异了，还没有选择
    # print(type(programs), len(programs)) <class 'list'> 5000
    return programs


class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):  # from sklearn base
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if not self.warm_start or not hasattr(self, '_programs'):
            ...
            self._programs = []
            # self._programs record every generation, _program[0] is the initial generation,
            # it's a list, every element is a formula
            # _program[1] is the first generation
        ...
        for gen in range(prior_generations, self.generations):
            ...
            if gen == 0:
                parents = None  # 在这里修改可以自定义初代种群? 或者在_Program初始化中修改
            else:
            ...
            # this is where the input X,y is used, to choose the formula with better performance
            # from the last generation, form the next generation
            ...
            self._programs.append(population)  # 将这一代种群添加到历史种群中
            # print(len(population)) 可以看到每一代种群数量都是5000
...
class SymbolicClassifier(BaseSymbolic, ClassifierMixin):
    ...
    def predict_proba(self, X):
        ...
        # self._transformer 是 sigmoid 函数或者是自定义的
