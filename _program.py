...
class _Program(object):
    ...
    def __init__(self,
        ...
        if self.program is not None:
            ...
            # sklearn 中的random_state与random_seed有一样的作用，
            # 为了避免生成随机数每次结果不同，导致无法调参或无法复现结果
            # random_state.randint() 相当于np.random.randint()
            # 差别在于random_state不同次执行会得到相同的结果，可复现
    ...
    def build_program(self, random_state):
        ...
        if self.init_method == 'half and half':
            ...
            # Trees are grown through a 50/50 mix of ‘full’ and ‘grow’,
            # making for a mix of tree shapes in the initial population.
        else:
            method = self.init_method
        max_depth = random_state.randint(*self.init_depth)
        # 在init_depth的范围里随机取值设定为当前公式的depth

        # Start a program with a function to avoid degenerative programs
        function = random_state.randint(len(self.function_set))
        function = self.function_set[function]
        program = [function]  # 从function_set中随机选取一个作为起点
        terminal_stack = [function.arity]

        # 逻辑：随机决定function和terminal
        # 从第一个函数开始，新生成的无论是function还是terminal
        # 都占了initial function的一个argument, 填满规则按从上到下，从左到右
        while terminal_stack:
            depth = len(terminal_stack)
            choice = self.n_features + len(self.function_set)  # 自变量个数加函数个数
            ...
            if (depth < max_depth) and (method == 'full' or
                ...
                if terminal == self.n_features:
                    ...
                    # 若取到了最后一个，取const_range内随机的float为terminal
                    # 前面的int 0，1,...就代表X0,X1,...
                ...
                while terminal_stack[-1] == 0:
                    terminal_stack.pop()  # 当前函数需要填满的argument已经填满
                    if not terminal_stack:
                        return program
                    terminal_stack[-1] -= 1  # 填满argument的函数完整占据了其母函数的一个argument，所以需要减1
                    # program is the list of element appearing in formula
                    # real [sub, add, -0.9992185259646544, 1, mul, sub, 1, 0, add, 0, 1]
                    # print sub(add(-0.999, X1), mul(sub(X1, X0), add(X0, X1)))
    ...
    def _depth(self):
        """Calculates the maximum depth of the program tree."""  # 官方例子的depth是3，节点层数-1
        terminals = [0]
        depth = 1
        for node in self.program:
            if isinstance(node, _Function):
    ...
    # return of execute is the value of input data X on the
    # formula represented by the current program
    def execute(self, X):
        ...
        for node in self.program:
            ...
            # 每个函数作为一个list,遇到自己的argument就放进自己的list里
            # 当一个函数的argument够了之后就赋值，并将结果返回上一个函数的argument
            while len(apply_stack[-1]) == apply_stack[-1][0].arity + 1:  # 这里加1是因为函数在第一位，后面才是argument
    ...
    def get_subtree(self, random_state, program=None):
        ...
        """
        # 这里的program是get_subtree的参数，只在函数内部用，和外部的program不冲突
        # 如果不调用这个参数，就代表对自己取subtree
        if program is None:
        ...
        # 这里将0.9和0.1改成9和1效果一样，赋予权重
        # 蒙特卡罗方法，random_state.uniform()相当于在[0,1]区间上投针

        stack = 1
        end = start
        while stack > end - start:  # 第一次肯定会进去
            node = program[end]  # 第一次相当于判断选中的node是function or terminal
            if isinstance(node, _Function):
                stack += node.arity
            end += 1  # 循环只要执行一次，end就要+1
            # 若选中的是terminal,循环执行一次结束,end=start+1
    ...
    def point_mutation(self, random_state):
        ...
        # random_state.uniform(size=len(program)生成
        # 长度和program一样的,取值在[0,1]的均匀分布的list，
        # mutate取出了值小于p_point_replace的点的index
        # 从这里看来，点变异在一个_program中可能发生在多个点，也有可能不发生
        ...
        for node in mutate:
            if isinstance(program[node], _Function):
                ...
                # arities是dict,第i个元是有i个输入的函数组成的list,see genetic.py line 350
            else:
                # We've got a terminal, add a const or variable
                if self.const_range is not None:
                    terminal = random_state.randint(self.n_features + 1)
                    # 与前面类似，若terminal是最后一个值，则取常数，否则为variable
                else:
                    terminal = random_state.randint(self.n_features)
                if terminal == self.n_features:
                    terminal = random_state.uniform(*self.const_range)
    ...
    # 这些是经过计算才得出的类的常量，非私有
    # 前面的_depth,_length,_indices是私有函数，不能在类外面被调用
